...
- シンボル操作はファイルを直接読む前にできるだけ Serena を使うこと
...
- ソースコードやドキュメントの追加・修正は最初に issue を作成し、それに対応する形で行う
...
- ソースコード修正は必ず最新の `main` ブランチからトピックブランチを作成しておこなう
- `main` ブランチでのコード修正は**厳禁**とする
- ソースコード修正時、異なるタイプの修正 (たとえば機能追加とリファクタリング) は極力コミットを分ける。
- Python のソースコードを追加・修正した場合、`ruff` と `mypy` でチェックし、警告がなくなるようにしておく
