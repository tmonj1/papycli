...
`papycli` は OpenAPI 3.0 仕様を読み込み、REST API エンドポイントをターミナルから直接呼び出せるインタラクティブな CLI を提供する Python 製ツールです。[xapicli](https://github.com/tmonj1/xapicli)（Bash 実装）の Python 移植版です。
...
xapicli の `get | post | put | delete` に加えて `patch` をサポートする。
...
|------|---------|---------|
| 実装言語 | Bash | Python |
| インストール方法 | スクリプトを source | pip install |
| 外部依存 | jq, GNU getopt, json-refs | なし（Python 標準 + requests） |
| $ref 解決 | json-refs (Node.js) | Python で内製 |
| サポートメソッド | GET/POST/PUT/DELETE | GET/POST/PUT/PATCH/DELETE |
| シェル補完 | bash のみ | bash / zsh |
| 設定ディレクトリ | `~/.xapicli` | `~/.papycli` |
| 環境変数プレフィックス | `XAPICLI_` | `PAPYCLI_` |
